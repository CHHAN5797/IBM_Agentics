# -*- coding: utf-8 -*-
"""DeFiLlama TVL MCP with protocol metadata caching and slug resolution."""
...
def _pairs_from_detail(detail: Dict[str, Any]) -> List[Tuple[int, float]]:
    """Normalize detail payload to [(unix_ts, tvl_usd), aggregating chains if needed]."""
...
def resolve_protocol_impl(
    ...
    refreshed = _refresh_meta(ttl_hours=0 if count == 0 else ttl_hours)

    hints = [hint, direct]
    base = _snapshot_base(direct)
    if base and base not in {h.lower() for h in hints}:
        hints.append(base)

    limit = None if top_n is None or top_n < 1 else int(top_n)
    candidates = rank_protocol_candidates(DB_PATH, hints, limit=limit)
    if candidates:
        return {"candidates": candidates, "refreshed": refreshed}

    if not refreshed:
        refreshed = _refresh_meta(ttl_hours=0)
        candidates = rank_protocol_candidates(DB_PATH, hints, limit=limit)
...
def refresh_protocol(slug: str) -> Dict[str, Any]:
    ...
    if not canon:
        raise ValueError("slug must be a non-empty string")
    if not slug_exists(DB_PATH, canon):
